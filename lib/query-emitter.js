...
QueryEmitter.prototype.updateQuery = function(query, ids){
  if (queryChanged(this.query, query) || !arrayEquals(this.ids, ids)) {
    if (this._polling) {
...
QueryEmitter.prototype.queryPoll = function(callback) {
  ...
  if (this._queryQueue.length) {
    this.query = this._queryQueue.shift();
  }
  ...
  this.db.queryPoll(this.collection, this.query, this.options, function(err, ids, extra) {
    ...
    if (idsDiff.length) {
      ...
      if (inserted.length) {
        emitter.backend.getSnapshotBulk(emitter.agent, emitter.collection, inserted, emitter.fields, function(err, snapshotMap) {
          if (err) return emitter._finishPoll(err, callback, pending);
...
QueryEmitter.prototype.queryPollDoc = function(id, callback) {
  ...
  this.db.queryPollDoc(this.collection, id, this.query, this.options, function(err, matches) {
    ...
    if (i === -1 && matches) {
      ...
      emitter.backend.getSnapshot(emitter.agent, emitter.collection, id, emitter.fields, function(err, snapshot) {
        if (err) return callback(err);
