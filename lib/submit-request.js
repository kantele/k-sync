...
SubmitRequest.prototype.submit = function(callback) {
  ...
  if (backend._skipDb[request.collection]) {
    return callback();
  }
...
SubmitRequest.prototype.commit = function(callback) {
  ...
  backend.trigger('commit', this.agent, this, function(err) {
    ...
    backend.db.commit(request.collection, request.id, request.opToDb || request.op, request.snapshot, null, function(err, succeeded) {
      if (err) return callback(err);
      if (!succeeded) {
