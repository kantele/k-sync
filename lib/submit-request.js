...
SubmitRequest.prototype.submit = function(callback) {
  ...
  backend.getSnapshot(this.agent, collection, id, fields, function(err, snapshot) {
...
SubmitRequest.prototype.commit = function(callback) {
  ...
  backend.trigger('commit', this.agent, this, function(err) {
    ...
    backend.db.commit(request.collection, request.id, backend.noOps[request.collection]? undefined: request.opToDb || request.op, request.snapshot, function(err, succeeded) {
      if (err) return callback(err);
      if (!succeeded) {
