...
function PathQueryEmitter(agent, backend, queryId, collection, query, options, callback) {
  ...
  async.series([
      function(callback) {
        self._subscribeBulk(self.collection, self.options && self.options.fetchOps || self.query, callback);
      },
      function(callback) {
        if (self.options && self.options.fetchOps) {
          self._subscribeBulk(self.collection, self.query, callback);  
        }
        else {
          callback(null, {});
        }
    ...
    function(err, results) {
      // combine the snapshotMaps
      // console.log('New PathQueryEmitter', {queryId, collection, query, options})
      // console.log('....results', err, results);
      var res = Object.assign(results[0] || {}, results[1] || {})
      self.removeUnusedIdsFromQuery(res);
      callback(null, { data: util.getResultsDataForMap(res, query) });
    }
  );

  // this._subscribeBulk(this.collection, this.options && this.options.fetchOps || this.query, function(err, snapshotMap) {
  //   this._subscribeBulk(this.collection, this.query, function(err, snapshotMap) {
