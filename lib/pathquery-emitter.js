...
function PathQueryEmitter(agent, backend, queryId, collection, query, callback) {
  this.agent = agent;
  this.backend = backend;
  this.queryId = queryId;
  this.collection = collection;
  this.query = query;
  this.agent._subscribeBulk(this.collection, this.query, callback);
}
...
PathQueryEmitter.prototype.updateQuery = function(query, callback) {
  ...
  function unsubscribe(cb) {
    if (diff2.removed.length) {
      self.agent._unsubscribeBulk(self.collection, diff2.removed, cb);
    }
    else {
      cb();
    }
