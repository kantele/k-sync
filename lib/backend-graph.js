...
//			var projection = backend.projections[to.collection],
//					collection = projection ? projection.target : to.collection,
//					fields = projection && projection.fields,
//					query = util.getQueryFromGraphData(data);
//
...
Backend.prototype.submitGop = function(agent, op, callback){
  ...
  backend.trigger('submit', agent, op, function(err) {
    if (err) return callback(err);
    request.run(function(err) {
      if (err) return callback(err);
      backend.trigger('after submit', agent, op, function(err) {
        callback(err);
      });
    });
  });
