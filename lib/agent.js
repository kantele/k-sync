...
var PathQueryEmitter = require('./pathquery-emitter');
...
Agent.prototype._cleanup = function() {
  ...
  for (var id in this.subscribedGraphs) {
    var emitter = this.subscribedGraphs[id];
    emitter.destroy();
  }
  // Cancel the subscribes
  this.subscribedGraphs = {};

  for (var id in this.subscribedNotifications) {
    var emitter = this.subscribedNotifications[id];
    emitter.destroy();
  }
  // Cancel the subscribes
  this.subscribedNotifications = {};
...
Agent.prototype.getExistingEmitter = function(queryId) {
  return this.subscribedQueries[queryId];
}

Agent.prototype._subscribeToQuery = function(emitter, queryId, collection, query) {
  var previousEmitter = this.subscribedQueries[queryId];
  if (previousEmitter) {
    if (previousEmitter === emitter) {
      return;
    }
    else {
...
Agent.prototype._checkRequest = function(request) {
  if (['ff', 'qf', 'qs', 'qu', 'gf', 'gs', 'gu', 'ns', 'nf', 'nu'].indexOf(request.a) !== -1) {
    // Query messages need an ID property.
    if (typeof request.id !== 'number') return 'Missing query ID';
  } else if (request.a === 'op' || request.a === 'gop' || request.a === 'nop' || request.a === 'f' || request.a === 's' || request.a === 'u') {
    // Doc-based request.
...
Agent.prototype._handleMessage = function(request, callback) {
  ...
  try {
    ...
    switch (request.a) {
      case 'rpc':
        return this._rpc(request.n, request.p, callback);
      case 'ff':
        return this._functionFetch(request.id, request.c, request.q, getQueryOptions(request), callback);
      case 'qf':
      ...
      case 'gs':
        return this._graphSubscribe(request, callback);
      case 'gf':
        return this._graphFetch(request, callback);
      case 'gu':
        return this._graphUnsubscribe(request, callback);
      case 'ns':
        return this._notificationSubscribe(request, callback);
      case 'nf':
        return this._notificationFetch(request, callback);
      case 'nu':
        return this._notificationUnsubscribe(request, callback);
      case 'bf':
      ...
      case 'gop':
        return this._submitGop(request, callback);
      case 'nop':
        return this._submitNop(request, callback);
      default:
...
Agent.prototype._rpc = function(name, params, callback) {
  function cb(err, model) {
    var data = model? {
      queries: model.root._queries.toJSON(),
      contexts: JSON.parse(JSON.stringify(model.root._contexts)),
      refs: model.root._refs.toJSON(),
      refLists: model.root._refLists.toJSON(),
      collections: model.serializeCollections(model.root),
      fns: model.root._fns.toJSON(),
      filters: model.root._filters.toJSON(),
      // nodeEnv: process.env.NODE_ENV
    }: {};

    callback(err, { a: 'rpc', n: name, b: data });
  }
  

  if (this.backend.rpc[name]) {
    this.backend.rpc[name](this.connectSession, params, cb);
  }
}

Agent.prototype._functionFetch = function(queryId, collection, query, options, callback) {
  // Fetch the results of a query once
  var agent = this;
  this.backend.functionFetch(this, collection, query, options, function(err, results, extra) {
    if (err) return callback(err);
    var message = {
      data: util.getResultsData(results),
      extra: extra
    };
...
Agent.prototype._pathquerySubscribe = function(queryId, collection, query, options, callback) {
  var emitter = this.getExistingEmitter(queryId);

  if (emitter) {
    emitter.updateQuery(query, callback);
  }
  else {
    this.subscribedQueries[queryId] = new PathQueryEmitter(this, this.backend, queryId, collection, query, options, callback)
  }
}

Agent.prototype._querySubscribe = function(queryId, collection, query, options, callback) {
  // If this is a path query, let's bulk subscribe to ids instead
  if (Array.isArray(query)) {
    this._pathquerySubscribe(queryId, collection, query, options, callback);
  }
  else {
...
Agent.prototype._fetch = function(collection, id, version, callback) {
  if (version == null) {
    ...
    this.backend.fetch(this, collection, id, function(err, snapshot) {
      if (err) return callback(err);
      if (!snapshot) return callback(new Error('snapshot empty: ' + collection + '.' + id));
