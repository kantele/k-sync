...
Query.prototype._handleResponse = function(err, data, extra) {
  ...
  if (Array.isArray(data)) {
    ...
    var results = this._ingestSnapshots(data, finish);
    if (results.length) {
      // new query?
      if (this.results.length == 0) {
        this.results = results;
      }
      // updating, go through the docs and add if not present already
      else {
        for (var j = 0; j < results.length; j++) {
          var found = false;

          for (var i = 0; i < this.results.length; i++) {
            if (this.results[i].id === results[j].id) {
              found = true;
            }
          }

          if (!found) {
            this.results.push(results[j]);
          }
        }
      }
    }
    ...
    for (var id in data) {
      ...
      var doc = this.connection.get(snapshot.c || this.collection, id, this.isGraph);
...
Query.prototype._ingestSnapshots = function(snapshots, finish) {
  ...
  for (var i = 0; i < snapshots.length; i++) {
    ...
    var doc = this.connection.get(snapshot.c || this.collection, snapshot.d, this.isGraph);
    doc.ingestSnapshot(snapshot, finish);
    results.push(doc);
  }
  return results;
