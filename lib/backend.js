...
Backend.prototype.querySubscribe = function(agent, index, query, options, callback) {
  if (Array.isArray(query)) {
    this.querySubscribePath(agent, index, query, options, callback);
  }
  else
  {
    this.querySubscribeNonPath(agent, index, query, options, callback);
  }  
}

Backend.prototype.querySubscribePath = function(agent, index, query, options, callback) {
  var start = Date.now();
  var backend = this;
  var queryEmitter;
  ...
  backend._triggerQuery(agent, index, query, options, function(err, request) {
    if (err) return callback(err);
    if (backend.db.disableSubscribe) return callback({message: 'DB does not support subscribe'});

    if (options.emitter) {
      queryEmitter = options.emitter;
      queryEmitter.updateQuery(query);
      ...
      backend.pubsub.subscribe(request.channel, function(err, stream) {
        ...
        queryEmitter = new QueryEmitter(request, stream, options.ids);
        backend.emit('timing', 'querySubscribe.reconnect', Date.now() - start, request);
        callback(null, queryEmitter);
      });
    }
...
Backend.prototype.querySubscribeNonPath = function(agent, index, query, options, callback) {
  var start = Date.now();
  var backend = this;
  var queryEmitter;
    ...
    backend.pubsub.subscribe(request.channel, function(err, stream) {
      ...
      backend._query(agent, request, function(err, snapshots, extra) {
        ...
        if (options.emitter) {
          ...
          queryEmitter = new QueryEmitter(request, stream, ids, extra);
        }
