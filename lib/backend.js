...
function Backend(options) {
  ...
  // Ops are not saved for these collections
  this._skipDb = {};

  // Map from name -> function
  this.functions = {};

  // Map from rpc function name -> fn
  this.rpc = {};
...
Backend.prototype.addRPC = function(name, fn) {
  this.rpc[name] = fn;
}

Backend.prototype.addFunction = function(name, code) {
  this.functions[name] = code;
}

// We may not want to save some collections to db.
// Used mainly for "after submit" hooks, i.e. user actions that are handled and discarded right away.
Backend.prototype.skipDb = function(collection) {
  this._skipDb[collection] = true;
}
...
Backend.prototype.getSnapshot = function(agent, collection, id, fields, callback) {
  var self = this;

  this.db.getSnapshot(collection, id, fields, function(err, snapshot) {
    self.trigger('dbread', agent, { collection, id, snapshot }, function() {
      callback(err, snapshot);
    });
  });
}

Backend.prototype.getSnapshotBulk = function(agent, collection, ids, fields, callback) {
  var self = this;

  this.db.getSnapshotBulk(collection, ids, fields, function(err, snapshotMap) {
    self.trigger('dbreadbulk', agent, { collection, ids, snapshotMap }, function() {
...
Backend.prototype.getOps = function(agent, index, id, from, to, callback) {
  ...
  backend.db.getOps(collection, id, from, to, function(err, ops) {
    if (err) return callback(err);
    self.trigger('dbopsread', agent, { collection, id, ops }, function() {
...
Backend.prototype.getOpsBulk = function(agent, index, fromMap, toMap, callback) {
  ...
  backend.db.getOpsBulk(collection, fromMap, toMap, function(err, opsMap) {
    if (err) return callback(err);
    self.trigger('dbopsbulkread', agent, { collection, opsMap }, function() {
...
Backend.prototype.fetch = function(agent, index, id, callback) {
  ...
  backend.getSnapshot(agent, collection, id, fields, function(err, snapshot) {
    if (err) return callback(err);
...
Backend.prototype.fetchBulk = function(agent, index, ids, callback) {
  ...
  backend.getSnapshotBulk(agent, collection, ids, fields, function(err, snapshotMap) {
    if (err) return callback(err);
    var snapshotProjection = backend._getSnapshotProjection(backend.db, projection);
  ...
  backend.pubsub.subscribe(channel, function(err, stream) {
    ...
    if (version == null) {
      ...
      backend.db.getOps(collection, id, version, null, function(err, ops) {
        if (err) return callback(err);
        backend.trigger('dbopsread', agent, { collection, id, ops }, function() {
  ...
  async.each(ids, function(id, eachCb) {
    ...
    if (doFetch) {
      ...
      backend.db.getOpsBulk(collection, versions, null, function(err, opsMap) {
        ...
        backend.trigger('dbopsbulkread', agent, { collection, opsMap }, function() {
...
Backend.prototype.functionFetch = function(agent, index, query, options, callback) {
  var start = Date.now();
  var backend = this;
  backend._triggerFunctionFetch(agent, index, query, options, function(err, request) {
    if (err) return callback(err);

    if (!backend.db.functionFetch) return callback({message: 'DB does not support functionFetch'});
    if (!backend.functions[request.fnname]) return callback({message: 'Stored function not found: ' + request.fnname});

    request.db.functionFetch(backend.functions[request.fnname], request.query.$p, function(err, snapshots, extra) {
      if (err) return callback(err);
      backend.emit('timing', 'functionFetch', Date.now() - start, request);
...
Backend.prototype._triggerFunctionFetch = function(agent, index, query, options, callback) {
  var request = {
    fnname: index,
    query: query,
    options: options,
  ...
  var backend = this;
  backend.trigger('FunctionFetch', agent, request, function(err) {
    if (err) return callback(err);
...
Backend.prototype._query = function(agent, request, callback) {
  ...
  request.db.query(request.collection, request.query, request.fields, request.options, function(err, snapshots, extra) {
    backend.trigger('dbreadbulk', agent, { collection: request.collection, snapshots }, function() {
      if (err) return callback(err);
